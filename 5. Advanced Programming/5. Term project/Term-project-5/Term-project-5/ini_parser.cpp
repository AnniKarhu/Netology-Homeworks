...
//конструктор //создание пустого невалидного объекта
...
ini_parser::ini_parser(std::string file_name)
{
	...
	fin.close();
	file_read = true; //хранит информацию о том, удалось ли считать данные из файла
...
bool ini_parser::check_parser() //проверяет, нет ли в парсере проблем и выбрасывает исключение
{
