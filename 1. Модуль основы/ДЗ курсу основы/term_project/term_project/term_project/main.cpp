...
#include <fstream>
#include <cstdlib>
//#include <unistd.h> //модуля нет на этом компе
...
int main()
{
	...
	while ((clear_console < 0) || (clear_console > 1))
	{
		std::cout << "Очищать консоль между выводами состояний?\n";
		std::cout << "0 - не очищать консоль\n";
		std::cout << "1 - очищать консоль, выводить через секунду\n\n";
	...
	while (state_changed && (alive_counter > 0))
	{
		...
		/*эта часть не работает, так как нет модуля unistd.h
		if (clear_console)
		{
			sleep(1);
			std::system("clear");
