...
bool graph::read_grapph_from_file(std::string file_name)
{
	...
	if (!fin.eof())
	{
		...
		if (temp > 0)
		{
			...
			stack_adjacency = new int[graph_size](); //массив стек - очередь прохождения элементов внутри одного компонента связности
...
int graph::dfs_adjacency(int vertex) //обход в глубину одной компоненты связности
{
	//static int dfs_counter = 0;
	//dfs_counter++;

	if ((vertex > (graph_size)) ||
		(vertex < 1)) // индекс массива не может быть меньше 0
	{
		return 0;
	}

	visited_vertices[vertex-1] = adjacency_num; //отметить вершину пройденной
	
	//записать вершину в стек	
	...
	for (int v = 0; v < graph_size; ++v)
	{
		if (vert_array[vertex-1][v]) //если существует ребро к смежной вершине 
		{
			if (!visited_vertices[v]) //если эту вершину еще не посещали
			{	
				dfs_adjacency(v+1); //запустить обход
			}			
		}
	}
	...
	while (stack_adjacency_index) //
	{
		...
		--stack_adjacency_index;
		dfs_adjacency(stack_adjacency[stack_adjacency_index]); //запустить обход уже пройденной вершины еще раз - чтобы вернуться к ответвлениям
	}	
	...
	for (int i = 0; i < graph_size; ++i)
	{
		...
		{
			++adjacency_num;
			dfs_adjacency(i+1); //запустить обход
		}
	}	
...
void graph::print_adjacency() //вывести принадлежность вершин компонентам связности
{
	std::cout << "Принадлежность вершин компонентам связности: \n";
	clear_visited(); //очистить предыдущие результаты, если уже запускались обходы		
	int num = dfs_adjacency(1); 	

	//теперь есть информация о  компонентах связности
	for (int i = 0; i < graph_size; ++i)
	{
		std::cout << i + 1 << " - " << visited_vertices[i] << "\n";
	}
	std::cout << "Количество компонентов связности в графе: " << num << "\n";
}