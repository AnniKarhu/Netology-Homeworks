...
Dynamic_array::Dynamic_array(unsigned int new_logical_size, unsigned int new_actual_size)
{
    ...
    {
        new_actual_size = new_logical_size; //фактический размер массива нельзя задавать меньше, чем логический размер
    }

    actual_size = new_actual_size;
    arr = new int[new_actual_size]();
...
int  Dynamic_array::operator[](const int elem_index)
{
    bool temp_bool = false; 
    return get_array_element(elem_index, temp_bool);  
}
